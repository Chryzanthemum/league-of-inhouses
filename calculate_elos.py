...
class Stats:
    def stats_to_network_graph(self, df):
        team_wins = (
            df[['game_id', 'team_id', 'win', 'ign']]
            .merge(
                df[['game_id', 'team_id', 'ign']],
    ...
    for g in games_df['league_game_id']:
        r = requests.get(f'https://na1.api.riotgames.com/lol/match/v4/matches/{g}', headers=header)
    ...
    network_graph_df = self.stats_to_network_graph(player_stats_df)
